...
一台網路設備每分鐘產生的大量資料難以逐條監控。而要能監控異常，最常見的做法是設定固定門檻，也最常失敗。監控門檻如何設定很依賴工程師經驗。門檻寬鬆，問題會太晚發現；門檻嚴格，則太多 false alarm，接著就開始忽略它們。這門課帶領大家一起從資料驅動的角度，從原始指標算出能解釋的偏離量，再依據偏離量動態決定告警時機。
...
課程教材放在 `labs/workshop/`。第六週 (預警與根因分析) 例外: 它整包放在 `week6_implementation/`,是一個解壓縮到桌面
就能從頭跑到底的獨立資料夾,學員當天拿到的就是它。當天第一件事是跑
[`week6_implementation/1_pkg_checker.ipynb`](week6_implementation/1_pkg_checker.ipynb),它會把缺的套件補齊並逐項檢查。
...
│   └── workshop/              # 課程教材
├── week6_implementation/      # 第六週整包，發給學員的獨立資料夾 (notebook、資料、投影片圖、
