...
從網路 telemetry 到 AIOps 決策支援的完整實作課程。學員在本機執行 Prometheus 與 exporter，選擇本機 Grafana 或 Grafana Cloud 作為視覺化層，完成特徵工程、異常偵測、告警降噪、預測與根因分析。

Grafana Cloud 路徑需要免費帳號（免安裝，瀏覽器即可使用）；本機路徑不需要帳號。課程著重演算法層面：特徵工程、偵測方法選擇、告警設計與根因判讀。
