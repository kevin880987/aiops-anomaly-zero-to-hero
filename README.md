...
一台網路設備每分鐘產生的指標，比任何人能逐條檢視的都多。而要能監控異常，最常見的做法是設定固定門檻，也最常失敗。監控門檻如何設定很依賴工程師經驗．門檻寬鬆，問題會太晚發現；門檻嚴格，則太多 false alarm，接著就開始忽略它們。這門課帶領大家一起從資料驅動的角度，從原始指標算出能解釋的偏離量，再決定多大的偏離值得發出告警。
