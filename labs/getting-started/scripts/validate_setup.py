...
def check_dashboard_metric_refs() -> tuple[bool, str]:
    exporter_source = ast.parse((REPO_ROOT / "infra/rrd_exporter.py").read_text(encoding="utf-8"))
    metric_cols = None
    ...
    if metric_cols is None:
        return False, "METRIC_COLS not found in rrd_exporter.py"
    ...
    if missing:
        return False, f"dashboard references metrics not emitted by rrd_exporter.py: {', '.join(missing)}"
    return True, "ok dashboard metric references match rrd_exporter.py"
