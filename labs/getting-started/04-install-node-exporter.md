...
本 repository 的 `prometheus/prometheus.yml` 已包含 macOS / Linux 的 node_exporter 目標：
...
Windows 使用 windows_exporter 時，預設 port 是 `9182`，而且指標名稱與 node_exporter 不完全相同。請不要改 `prometheus/prometheus.yml`，直接使用 `prometheus/prometheus.windows.yml`。初學者若使用 Windows，建議先完成 full-course 合成資料路徑；若要跑 hands-on 即時 OS 指標，依 notebook 提示調整 PromQL 指標名稱。
...
有結果（至少看到 `en0` 或 `eth0`）即可進入 `labs/hands-on/00_observability_stack_and_promql.ipynb`。
