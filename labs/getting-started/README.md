...
Step 3   開啟 Labs，開始課程
         ...
         實戰工作坊版（hands-on）
            ...
            labs/hands-on/01_network_traffic_feature_engineering.ipynb
            labs/hands-on/02_anomaly_detection_and_alerting.ipynb
            labs/hands-on/08_agentic_ai_rca_capstone.ipynb

         完整自學版（9 labs）
            labs/full-course/00_observability_stack.ipynb
            labs/full-course/01_time_series_features.ipynb
            … labs/full-course/08_deploy_to_production.ipynb
```
...
Prometheus 是本機監控服務，不安裝在 conda 環境裡。課程的 `exporter.py` 則使用 conda 環境中的 `prometheus_client`，把合成資料轉成 Prometheus 可抓取的 metrics。macOS / Linux 使用 `prometheus/prometheus.yml`。Windows 使用 `prometheus/prometheus.windows.yml`，因為 Windows 的 exporter 預設 port 與 metrics 名稱不同。
...
- 工作坊短版：從 `labs/hands-on/00_observability_stack_and_promql.ipynb` 開始。
- 完整自學版：從 `labs/full-course/00_observability_stack.ipynb` 開始。
...
| 01b | [01b-setup-windows-python-environment.md](01b-setup-windows-python-environment.md) | Windows conda 環境建立與 JupyterLab 啟動 |
