...
def plot_series(g, half, cadence, period, budget, scores=None, bands=None, title=None,
                window=None):
    """One panel per detector, ground truth shaded, TP/FP/FN marked, on one port with several
    candidate detectors stacked instead of several columns compared across ports.

    `scores` is a dict {name: array}: the statistic itself, with a horizontal cutoff at `budget`'s
    quantile and its alerts split into TP/FP/FN.

    `bands` is a dict {name: (signal, lower, upper)}: `signal` is the column to draw (e.g.
    `g["value"]` or `g["resid"]`), `lower`/`upper` the band edges in that column's own units,
    each a Series or a plain number. A cell can pass both — a band on the signal for the
    reference level a statistic like CUSUM or EWMA is built on, and the statistic itself in its
    own panel below.

    `window` is a (start, end) pair; the default zooms to the anomaly windows plus one `period` on
    each side. The TP/FP/FN counts in every panel title are taken over the whole series
    regardless of the zoom, the same rule `report` uses, so a title and a table never disagree.
    """
    scores, bands = scores or {}, bands or {}
    ...
    pos = g["target"].to_numpy()

    def mark_outcomes(ax, s, fired, x):
        for kind, m in (("TP", fired & pos), ("FP", fired & ~pos), ("FN", ~fired & pos)):
            colour, marker = CONF[kind]
            mm = m & view
            ax.plot(x[mm], s[mm], linestyle="none", marker=marker, color=colour, zorder=5)
        tally = {"TP": int((fired & pos).sum()), "FP": int((fired & ~pos).sum()),
                 "FN": int((~fired & pos).sum())}
        ax.set_title("  ".join(f"{k} {v}" for k, v in tally.items()),
                     fontsize=plt.rcParams["legend.fontsize"])

    n = len(bands) + len(scores)
    w, h = plt.rcParams["figure.figsize"]
    fig, axes = plt.subplots(n, 1, figsize=(w, h * 0.72 * n), sharex=True, squeeze=False)
    x = g["timestamp"]
    i = 0
    for name, (signal, lower, upper) in bands.items():
        ax = axes[i, 0]; i += 1
        s = pd.Series(np.asarray(signal, dtype=float), index=g.index)
        ax.plot(x[view], s[view], marker=mark, color=C["signal"])
        lo = (pd.Series(float(lower), index=s.index) if np.isscalar(lower)
              else pd.Series(lower).reindex(s.index))
        up = (pd.Series(float(upper), index=s.index) if np.isscalar(upper)
              else pd.Series(upper).reindex(s.index))
        ax.fill_between(x[view], lo[view], up[view], color=C["band"], alpha=BAND_ALPHA,
                        lw=0, zorder=0)
        fired = ((s < lo) | (s > up)).fillna(False).to_numpy()
        mark_outcomes(ax, s, fired, x)
        ax.set_ylabel(name)

    for name, sc in scores.items():
        ax = axes[i, 0]; i += 1
        s = pd.Series(sc, dtype=float).reindex(range(len(g)))
        thr = np.nanquantile(s.dropna(), budget)
        ax.plot(x[view], s[view], color=C["score"])
        ax.axhline(thr, color=C["alert"], ls="--")
        fired = (s > thr).fillna(False).to_numpy()
        mark_outcomes(ax, s, fired, x)
        ax.set_ylabel(name)
    ...
    handles = [plt.Line2D([], [], color=C["signal"], label="signal"),
              plt.Rectangle((0, 0), 1, 1, color=C["truth"], alpha=0.25,
                            label="anomaly (ground truth)")]
    if bands:
        handles.append(plt.Rectangle((0, 0), 1, 1, color=C["band"], alpha=BAND_KEY_ALPHA,
                                     label="no alert inside this band"))
    handles += [plt.Line2D([], [], linestyle="none", marker=marker, color=colour,
                           label=CONF_LABEL[kind]) for kind, (colour, marker) in CONF.items()]
    fig.legend(handles=handles, labels=[a.get_label() for a in handles], ncol=len(handles),
              loc="outside lower center", framealpha=0.9, columnspacing=1.1,
              handletextpad=0.5)
