...
每個 full-course notebook 會讀取前一步輸出，並把新的中間結果寫回 `data/processed/`。如果中途失敗，先從失敗 notebook 的前一個 lab 重跑，不要直接跳到後面的 lab。
...
若時間不足，保留 `hands-on/00`、`hands-on/01`、`hands-on/02`，把 `hands-on/08` 作為展示或課後作業。完整自學版則適合要求學員提交 `data/processed/` 產出的 CSV 與一段 RCA 判讀說明。
